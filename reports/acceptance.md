...
- **최종 바이너리 크기**: **43,008 바이트** (42.00 KB / 0.0410 MiB)
- **크기 목표(8,000,000 바이트 / 7~8MB 이하)**: **합격 [PASS]** (목표치의 0.5% 수준)
- **SHA-256 해시**: `796ba8869aa9a23b898ef6b2920eef37e86e42e09a900df504f9ec41c59b90f6`
...
| **AC-02** | 열기/닫기/종료 및 단축키(F2/F4/F8), 문서 교체/초기화 | **PASS** | `[열기 (F2)]` 및 F2 키로 MSG 파일 선택 다이얼로그 호출, `[닫기 (F4)]` 및 F4 키로 현재 문서 정리 후 빈 뷰어 전환, `[종료]` 및 창 X 즉시 종료 확인. |
...
| **AC-08** | EXE 크기 및 실행/열기/종료 시간 실측 | **PASS** | 파일 크기 43,008 바이트 (목표 8MB 이하 달성). 초기 실행 및 열기 지연시간 ~20ms 이내 실측. 상세 데이터는 `reports/performance.csv` 참조. |
...
| **AC-10** | 4개 국어 지원, 고정 문구 버튼 및 F8 언어 순환 | **PASS** | `[언어 / Language / Langue / 言語 (F8)]` 단일 고정 버튼 배치. 마우스 클릭 시 4개 언어 메뉴 팝업 및 닫힘 시 버튼 눌림 복원 확인. F8 키보드 입력 시 한국어→영어→프랑스어→일본어 즉시 순환 검증 완료. |
| **AC-11** | 헤더 6종 레이아웃 및 빈 필드(받는 사람/제목) '없음' 표기 | **PASS** | 보내는 사람(좌) + 전송일시(`YYYY-MM-DD HH:mm:ss`, 우), 받는 사람, 참조, 숨은참조(정보 없음), 제목 레이아웃 검증. 받는 사람 및 제목이 비어 있는 경우 다국어 `없음`(`None`, `Aucun`, `なし`) 명시 표기 확인. |
